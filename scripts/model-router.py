...
def classify_task(prompt):
    ...
    for pattern in BACKGROUND_TASK_PATTERNS:
        if re.search(prompt, prompt_lower):
...
def main():
    ...
    if len(sys.argv) < 2:
        print("Usage: model-router.py <command> [args]")
        print("")
        print("Commands:")
        ...
        print("")
        print("Examples:")
        print("  model-router.py route 'thanks!'")
        print("  model-router.py route 'design an architecture' --provider openai")
        print("  model-router.py compare")
        sys.exit(1)
    ...
    if command == "route" or command not in known_commands:
        ...
        if command == "route":
            if len(sys.argv) < 3:
                print("Usage: model-router.py route '<prompt>'")
                sys.exit(1)
            prompt = sys.argv[2]
            start_idx = 3
        else:
