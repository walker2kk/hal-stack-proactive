...
def plan_heartbeat(checks=None):
    ...
    result["cache_ttl_tip"] = (
        ...
        "Run: heartbeat-optimizer.py cache-ttl for details."
    )
...
def main():
    ...
    if len(sys.argv) < 2:
        print("Usage: heartbeat-optimizer.py [plan|check|record|interval|cache-ttl|reset]")
        sys.exit(1)
    ...
    if command == "plan":
        ...
        if len(sys.argv) < 3:
            print("Usage: heartbeat-optimizer.py check <type>")
            sys.exit(1)
        ...
        # Record that a check was performed
        if len(sys.argv) < 3:
            print("Usage: heartbeat-optimizer.py record <type>")
            sys.exit(1)
        check_type = sys.argv[2]
        ...
        # Update interval
        if len(sys.argv) < 4:
            print("Usage: heartbeat-optimizer.py interval <type> <seconds>")
            sys.exit(1)
        check_type = sys.argv[2]
