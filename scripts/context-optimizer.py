...
def main():
    ...
    if len(sys.argv) < 2:
        print("Usage: context-optimizer.py [recommend|record|stats|generate-agents]")
        sys.exit(1)
    ...
    if command == "recommend":
        if len(sys.argv) < 3:
            print("Usage: context-optimizer.py recommend '<prompt>' [current_files]")
            sys.exit(1)
        ...
        if len(sys.argv) < 3:
            print("Usage: context-optimizer.py record <file_path>")
            sys.exit(1)
